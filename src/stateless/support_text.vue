...
export default {
    ...
    methods: {
        ...
        open (ev) {
            this.showText = true
            this.$nextTick(() => {
                this.expandedHeight = this.$refs.content.scrollHeight
                this.isMultiline = this.$refs.content.scrollHeight !== this.$refs.content.clientHeight

                this.$refs.content.addEventListener('mouseleave', this.close)
                this.$refs.content.style.transform = 'translateX(0px)'
                this.timeoutId = setTimeout(() => {
                    this.$refs.content.style.height = `${this.expandedHeight - 2 }px` // 2px of padding
                }, this.isMultiline ? 100 : 150)
            })
        },
        close (ev) {
            ...
            setTimeout(() =>
                this.$refs.content.style.transform = `translateX(${this.isRight ? 225 : -225}px)`, this.isMultiline ? 80: 150
            )
